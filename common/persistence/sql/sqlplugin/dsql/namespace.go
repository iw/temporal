...
func (pdb *db) UpdateNamespace(
	...
	// Log the row data for debugging
	fmt.Printf("[DSQL-DEBUG] UpdateNamespace: ID=%s, Name=%s, DataType=%T, DataLen=%d, DataEncoding=%s, DataHex=%x\n",
		row.ID, row.Name, row.Data, len(row.Data), row.DataEncoding, row.Data)
 ...
 WHERE partition_id=54321 AND id = $6`
	...
	return pdb.ExecContext(ctx, query, row.Name, row.Data, row.DataEncoding, row.IsGlobal, row.NotificationVersion, idStr)
}
