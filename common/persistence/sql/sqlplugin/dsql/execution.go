...
func (pdb *db) UpdateExecutions(
 ...
 WHERE shard_id = $8 AND namespace_id = $9 AND workflow_id = $10 AND run_id = $11`,
		row.DBRecordVersion,
		row.NextEventID,
		row.LastWriteVersion,
		row.Data,
...
VALUES ($1, $2, $3, $4, $5, $6)`,
	...
	// Return a dummy result since we can't return the actual result from multiple inserts
	return nil, nil
}
